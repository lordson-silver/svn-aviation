      ...
      <div className="max-w-[1400px] mx-auto px-8 md:px-12 relative z-10">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-20 gap-8">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl md:text-6xl font-serif">Capability & Coverage</h2>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
           {opsHubs.map((hub, i) => (
             ...
             >
               <h3 className="text-2xl font-serif text-brand-yellow mb-2">{hub.name}</h3>
               <p className="text-[10px] tracking-[0.2em] font-black uppercase text-white/40">{hub.hub}</p>
             </motion.div>
           ))}
        </div>
        ...
        <div className="mt-20 p-12 border border-white/5 bg-brand-navy/20 backdrop-blur-sm max-w-3xl">
           <p className="text-lg text-white/70 italic font-serif leading-relaxed">
             &quot;Precision aviation coordination requires more than aircraft access. 
             It requires operational structure, responsiveness, and regulatory alignment.&quot;
           </p>
        </div>
      </div>
    </section>
